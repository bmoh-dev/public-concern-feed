...
export const Route = createFileRoute("/_authenticated/platform-municipalities")({
  head: () => ({ meta: [{ title: "اعتماد البلديات" }] }),
  beforeLoad: async () => {
    try {
  ...
  component: PlatformMunicipalitiesPage,
});

function PlatformMunicipalitiesPage() {
  ...
  const refresh = () =>
    qc.invalidateQueries({ queryKey: ["platform-admin-municipalities"] });
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-semibold">
                    {m.name}{" "}
                    <span className="text-muted-foreground">— {m.wilaya}</span>
                  </div>
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          if (reason.trim().length < 3)
                            return toast.error("اذكر سبباً واضحاً");
                          try {
                            await rejectFn({
